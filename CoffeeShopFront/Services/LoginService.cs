...
namespace CoffeeShopFront.Services
{
    public class LoginService
    {
        ...
        private readonly CoffeeAPI _backendApiHttpClient;

        public LoginService(ProtectedLocalStorage localStorage, NavigationManager navigation, IConfiguration configuration, CoffeeAPI backendApiHttpClient)
        {
        ...
        {
            var response = await _backendApiHttpClient.Login(model);
            if (string.IsNullOrEmpty(response?.Content))
                return false;

            await _localStorage.SetAsync(AccessToken, response.Content);
